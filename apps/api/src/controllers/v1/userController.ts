...
export const signUp = asyncHandler(async (req, res) => {
  ...
  if (!parsedSignupData.success) {
    throw AppError.badRequest(
      parsedSignupData.error.issues[0].message || "Invalid input",
      { field: parsedSignupData.error.issues[0].path.join(".") },
    );
  }
  ...
  if (existingUser) {
    throw AppError.badRequest("User with this email or name already exists", {
      field: ["email", "name"],
    });
  }
...
export const signIn = asyncHandler(async (req, res) => {
  ...
  if (!parsedSignInData.success) {
    throw AppError.badRequest(
      parsedSignInData.error.issues[0].message || "Invalid input",
      { field: parsedSignInData.error.issues[0].path.join(".") },
    );
  }
