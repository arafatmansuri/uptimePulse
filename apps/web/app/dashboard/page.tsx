...
const statusConfig: Record<WebsiteStatus, { label: string; dot: string; text: string; badge: string }> = {
  [WebsiteStatus.UP]: {
    label: 'Operational',
    dot: 'bg-brand-400',
    text: 'text-brand-300',
...
function WebsiteRow({ website }: { website: Website }) {
  console.log("Log from main web row",website);
  const cfg = statusConfig[website.ticks[0].status];
  return (
    <div className="flex items-center justify-between gap-4 border-b border-ink-800/60 px-5 py-4 transition hover:bg-ink-900/40 last:border-b-0">
      ...
      <div className="hidden flex-shrink-0 items-center gap-6 sm:flex">
        ...
        <div className="text-right">
          ...
          <div className="flex items-center gap-1 text-sm font-medium text-ink-200">
            ...
            {timeAgo(website.ticks[0].createdAt.toISOString())}
          </div>
        </div>
      </div>
...
export default function DashboardPage() {
  ...
  const { isLoading: isAuthLoading, isError: isAuthError } = useAuthQuery({endpoint:"/me"});
  ...
  useEffect(()=>{
    ...
    //   setWebsites(data?.data.websites || []);
      console.log("Log from main comp",websites);
    }
  },[isLoading,isError]);

  const operational = websites.filter((w) => w.ticks[0].status === WebsiteStatus.UP).length;
  const avgResponse =
    websites.length > 0
      ? Math.round(websites.reduce((sum, w) => sum + w.ticks[0].response_time_ms, 0) / websites.length)
      : 0;
  const downCount = websites.filter((w) => w.ticks[0].status === WebsiteStatus.DOWN).length;
  ...
  return (
    <div className="min-h-screen bg-ink-950 text-ink-100">
      ...
      <main className="mx-auto w-full max-w-7xl px-5 py-8 sm:px-8">
        ...
        <p className="mt-1 text-sm text-ink-400">Monitor your websites and endpoints in real time.</p>

        {/* Stats */}
        <div className="mt-6 grid grid-cols-2 gap-4 lg:grid-cols-4">
          <StatCard icon={Server} label="Total monitors" value={String(websites.length)} tone="text-ink-200" />
          <StatCard icon={Activity} label="Operational" value={String(operational)} tone="text-brand-400" />
          <StatCard icon={Zap} label="Avg response" value={`${avgResponse}ms`} tone="text-amber-400" />
          <StatCard icon={Activity} label="Down" value={String(downCount)} tone="text-red-400" />
        </div>
        ...
        <div className="mt-8 rounded-2xl border border-ink-800 bg-ink-900/40">
          <div className="flex items-center justify-between border-b border-ink-800/60 px-5 py-4">
            ...
            {websites.length > 0 && (
              <span className="text-xs text-ink-500">{websites.length} monitor{websites.length !== 1 ? 's' : ''}</span>
            )}
          </div>

          {isLoading ? (
            <div className="flex items-center justify-center gap-2 py-16 text-ink-400">
              <Loader2 className="h-5 w-5 animate-spin" /> Loading your monitors...
            </div>
          ) : isError ? (
            <div className="px-5 py-16 text-center">
              <p className="text-sm text-red-400">{(error.message) || 'Failed to load websites.'}</p>
            </div>
          ) : websites.length === 0 ? (
            <div className="flex flex-col items-center justify-center gap-3 py-16 text-center">
              ...
              <div>
                <p className="text-sm font-semibold text-white">No monitors yet</p>
                <p className="mt-1 text-sm text-ink-400">Add your first website to start tracking uptime.</p>
              </div>
              <Button size="sm" onClick={() => setModalOpen(true)} className="mt-2">
                <Plus className="h-4 w-4" /> Add website
              </Button>
            </div>
          ) : (
